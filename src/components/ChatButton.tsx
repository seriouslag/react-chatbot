...
import type { ChatService } from '../service/ChatService';
import { ChatBox, type ChatBoxOptons } from './ChatBox';
import './index.css';
...
type ChatOptions = Pick<
  ChatBoxOptons,
  'botName' | 'botAvatar' | 'botDescription' | 'inputTextPlaceholder' | 'sendButtonText'
>;
...
export const ChatButton = ({ service, ...rest }: ChatButtonProps) => {
  ...
  useEffect(() => {
    ...
    if (newMessage) {
      ...
      newMessage.scrollIntoView({ behavior: firstRender ? 'instant' : 'smooth' });
    }
