...
export const ChatBox = ({
  ...
  const calculatedBotAvatar = useMemo(() => botAvatar || <SmartToyIcon />, [botAvatar]);
  const calculatedUserAvatar = useMemo(() => userAvatar || <SmartToyIcon />, [userAvatar]);
      ...
      >
        <div className="flex justify-between border-b-2 border-gray-200 py-3 sm:items-center">
          <div className="relative flex items-center space-x-4">
            <div className="relative">
              ...
              <span className={`absolute -bottom-1.5 -right-1.5 ${statusColor}`}>
            ...
            <div className="flex flex-col leading-tight">
              ...
              {botDescription && <span className="text-lg text-gray-600">{botDescription}</span>}
            </div>
          </div>
        </div>
        <div
        ...
        >
          {messages.map((message) => (
            <div className="chat-message" key={message.id} id={`message-${message.id}`}>
              {message.role === 'user' ? (
                <MyMessageBubble message={message} userAvatar={calculatedUserAvatar} />
              ) : (
                <YourMessageBubble message={message} botAvatar={calculatedBotAvatar} />
              )}
            </div>
          ))}
        </div>
