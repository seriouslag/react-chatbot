...
import type { ChatService, ChatState, MyMessage, SystemMessage } from './ChatService';
    ...
    try {
      ...
      for await (const event of readNDJSONStream(response)) {
        if (event?.choices?.[0]?.context?.data_points) {
          ...
          const currentMessage = this.#state.value.messages.filter((m) => m.id === responseId)?.[0]?.message ?? '';
      ...
      const finalMessage = this.#state.value.messages.filter((m) => m.id === responseId)?.[0]?.message;
      if (!finalMessage) {
